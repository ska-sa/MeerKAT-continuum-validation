...
class catalogue(object):
    ...
    def fit_spectra(self, cat_name=None, match_perc=0, models=['pow'], fig_extn=None,
        ...
        """Derive radio spectra for this catalogue, using the input SED models.
        This will add new columns to the table, including the spectral index
        and error, and optionally, the fitted flux at the frequency of this
        instance and the ratio between this and the measured flux.

        Keyword arguments:
        ------------------
        cat_name : string
            Derive spectral indices between the catalogue given by this dictionary key and
            the main catalogue from this instance. If None is input, all data except the
            catalogue from this instance will be used, and the flux at its frequency will
            be derived. If 'all' is input, all data will be used.
        match_perc : float
            Don't use the fluxes from a catalogue if the number of sources is less than this
            percentage of the main catalogue. Only used when cat_name is None. Use 0 to accept
            all catalogues.
        models : list
        ...
        GLEAM_subbands : string
            Use all GLEAM sub-band measurements, if GLEAM cross-matched. Input 'int' for
            integrated fluxes,
            'peak' for peak fluxes, and None to use none.
        GLEAM_nchans : int
            Average together this many 8 MHz GLEAM sub-bands. Use None for no averaging.
        fit_flux : bool
            Use all cross-matched catalogues to derive a fitted flux. If False, a typica
            l spectral index of -0.8 will be assumed.
        redo : bool
            Derive spectral indices, even if the file exists and the spectral indices have
            been derived from these frequencies.
        write : bool
        ...
        if cat_name in [None, 'all']:
            ...
            # otherwise, derive the spectral index and fitted flux using all available frequencies
                ...
                self.n_point_spectra(fitted_flux_suffix, fitted_ratio_suffix, best_fitted_flux,
                                     best_fitted_ratio, used_cats, freq, models=models,
    ...
    def est_fitted_flux(self, fitted_flux_col, fitted_ratio_col, freq, cat, spec_index=-0.8):
        ...
        if self.verbose:
            print("Deriving the flux at {0} MHz assuming a typical spectral index of {1}.".format(
                freq, spec_index))

        self.df[fitted_flux_col] = flux_at_freq(self.freq[self.name], self.freq[cat],
                                                self.flux[cat], spec_index)
    ...
    def two_point_spectra(self, cat_name, redo=False):
        ...
        ----------
        cat_name : string
            Derive spectral indices between the catalogue given by this dictionary key
            and the catalogue from this instance.
        ...
        if redo or alpha_col not in self.df.columns:
            ...
            if np.abs(self.freq[cat_name]/self.freq[self.name]-1) <= 0.1:
                ...
                self.df[alpha_col], self.df[alpha_err_col], flux = two_freq_power_law(
                    self.freq[self.name], [self.freq[self.name], self.freq[cat_name]],
    ...
    def n_point_spectra(self, fitted_flux_suffix, fitted_ratio_suffix, best_fitted_flux,
        ...
        """Derive the radio spectra from the input SED models, using the specified data,
        presumed to be >2 frequency measurements.
        ...
        ------------------
        cat_name : string
            If None is input, all data except the catalogue from this instance will
            be used, and the flux at the frequency given by this instance will be
            derived from these spectral indices. If 'all' is input, all data will be used.
        models : list
        ...
        GLEAM_nchans : int
            Average together this many 8 MHz GLEAM sub-bands. Use None for no averaging.
        redo : bool
            Derive spectral indices, even if the file exists and the spectral indices
            have been derived from these frequencies."""
            ...
            if GLEAM_subbands is not None and 'GLEAM' in self.cat_list:
                ...
                for col in self.df.columns:
                    if col.startswith('GLEAM_{0}_flux_'.format(GLEAM_subbands)) and \
                        ...
                        GLEAM_fluxes = np.append(GLEAM_fluxes,
                                                 self.df.loc[i, 'GLEAM_{0}_flux_{1}'.format(
                                                     GLEAM_subbands, GLEAM_freq)])
                        GLEAM_errs = np.append(GLEAM_errs,
                ...
                if GLEAM_nchans is not None:
                    ...
                    while index < len(GLEAM_freqs) and GLEAM_freqs[index+GLEAM_nchans] <= 174:
                        ...
                        GLEAM_errs[used_index] = np.sqrt(np.sum(GLEAM_errs[
                            index:index+GLEAM_nchans]**2)) / GLEAM_nchans
                        index += GLEAM_nchans
                        used_index += 1
    ...
    def process_config_file(self, config_file, main_dir, redo=False, write_all=True,
        ...
        """For a given catalogue config file, read the paramaters into a dictionary,
        pass it into a new catalogue object, cut out a box, cross-match to this instance,
        and derive the spectral index.
        ...
        # Cut out a box in catalogues within boundaries of image, cross-match and derive
        # spectral indices
